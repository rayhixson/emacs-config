...
(defcustom eshell-git-prompt-themes
  '((rays
     eshell-git-prompt-rays
     eshell-git-prompt-rays-regexp)
    (robbyrussell
...
(defun eshell-git-prompt-rays ()
  "Eshell Git prompt with some hacks 
...
└─> $ "
  ...
  (let (git-branch git-dirty)
    ...
    (concat
     ...
     (propertize "]──[" 'face `(:foreground "green"))
     (propertize (concat git-branch) 'face `(:foreground "cyan"))
     (propertize (concat git-dirty "]──[") 'face `(:foreground "green"))
